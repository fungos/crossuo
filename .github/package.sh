...
if [[ "$RUNNER_OS" == "Linux" ]]; then
  if [[ "$COMPILER" == "clang" ]]; then
      ...
      cp src/libcrossuo.so crossuo-linux-$BUILD/crossuo.so
      cp src/crossuo crossuo-linux-$BUILD/
      cp tools/xuoi/xuolauncher crossuo-linux-$BUILD/
    ...
    ls -la build/
    ls -la build/src/
    cp build/libcrossuo.dylib CrossUO.app/Contents/MacOS/
